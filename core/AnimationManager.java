...
public class AnimationManager {
    ...
    /**
     * Returns the size of the first frame of the given animation (packedWidth × packedHeight).
     * Used during character initialisation to set the hitbox size.
     *
     * @param name the animation key
     * @return a {@link Vector2D} with the frame's packed dimensions, or {@code (64, 64)} as fallback
     */
    ...
    /**
     * Returns the maximum size (width × height) across ALL frames of the given animation,
     * using the packed dimensions of each sprite in the atlas.
     *
     * <p>Some animations have frames of varying sizes (e.g. a billowing cloak).
     * Using the maximum produces a stable bounding box for the entire animation
     * without visual "jumping".
     *
     * @param name the animation key
     * @return a {@link Vector2D} with the maximum packed dimensions, or {@code (64, 64)} as fallback
     */
    ...
    /**
     * Returns the total duration of the named animation in seconds.
     *
     * @param name the animation key
     * @return total animation duration, or {@code 0f} if not found
     */
    ...
    /**
     * Returns the number of frames in the named animation.
     *
     * @param name the animation key
     * @return frame count, or {@code 1} as fallback if not found
     */
    ...
    /** Returns the key of the currently playing animation, or {@code null} if none is active. */
    public String getCurrentAnimation() {
        return currentAnimation;
    }

    /**
     * Returns the current animation timer value.
     * Used by {@code AnimationRenderer} when computing frame-based offsets.
     */
    ...
    /**
     * Returns {@code true} if an animation with the given key has been registered.
     *
    ...
    /**
     * Clears all registered animations and frame-duration entries.
     * The atlas itself is managed by {@link AtlasCache} and is not disposed here.
     */
    public void dispose() {
        animations.clear();
