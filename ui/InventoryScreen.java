...
public class InventoryScreen implements Screen {
     ...
     */
    public InventoryScreen(AppController app, int levelToStart, Inventory inventory) {
        this.app          = app;
    ...
    private void buildAvailable() {
        ...
        for (PlayerCharacter c : pool) {
            boolean alreadyIn = this.inventory.getCharacters().stream()
    ...
    private void rebuildButtons() {
        ...
        y = 390f;
        for (int i = 0; i < this.inventory.getCharacters().size(); i++, y -= ROW_H) {
            remCharBtns.add(new Rectangle(MID + 320, y - BTN_H + 4, BTN_W, BTN_H));
        }

        y = 230f;
        java.util.LinkedHashMap<Class<?>, List<Item>> grouped = groupItems(this.inventory);
    ...
    private void drawText(float mx, float my) {
        ...
        drawAvailableChars();
        drawAvailableItems();
        drawInventoryChars(this.inventory);
        drawInventoryItems(this.inventory);
    ...
    private void drawInventoryChars(Inventory inv) {
        float y = 390f;
        List<PlayerCharacter> chars = this.inventory.getCharacters();
        for (int i = 0; i < chars.size(); i++, y -= ROW_H) {
            PlayerCharacter c      = chars.get(i);
            boolean         isBase = this.inventory.isBaseCharacter(c);
